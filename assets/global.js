...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    if (!this.currentVariant) {
      ...
      var getlocw = window.location.href;
      var $curvar = this.currentVariant;
      if(getlocw.indexOf('landing-page') > -1){
      $('.curvrperc').each(function(){
        if($(this).attr('data-id') == $curvar.id){
          var $getpric = $(this).attr('data-price');
          var $getcomppric = $(this).attr('data-compprice');
          var $remcurrpric = $getpric.split('$');
          var $remcurrcomppric = $getcomppric.split('$');
          var diffpr = $remcurrcomppric[1] - $remcurrpric[1];
          var gttl = diffpr*100;
          var gdiv = gttl/$remcurrcomppric[1];
          console.log(gdiv);
          
           setTimeout(function(){
             if($('.percentpric').length > 1){
              $('.percentpric').html(Math.trunc(gdiv) +' % off');
             }
             else{
                $('.price__sale').append('<span class="percentpric">'+ Math.trunc(gdiv) +' % off</span>');
             }
            },1200); 
        }
      });
